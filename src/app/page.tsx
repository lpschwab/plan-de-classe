...
export default function Home() {
  ...
  const [classeEnRenommageId, setClasseEnRenommageId] = useState<string | null>(
    null,
  );

  const [nomClasseEnEdition, setNomClasseEnEdition] = useState("");
  ...
  */
  ...
  function commencerRenommageClasse(projet: Projet) {
    setClasseEnRenommageId(projet.id);
    setNomClasseEnEdition(projet.nom);
  }

  function annulerRenommageClasse() {
    setClasseEnRenommageId(null);
    setNomClasseEnEdition("");
  }

  function enregistrerNouveauNomClasse(projetId: string) {
    const nouveauNom = nomClasseEnEdition.trim();

    if (!nouveauNom) {
      return;
    }

    const nouvelleListe = projets.map(function (projet) {
    ...
    // Si la classe actuellement ouverte est renommée depuis "Mes classes",
    // on garde également son nom courant synchronisé.
    if (projetActifId === projetId) {
      setNomProjet(nouveauNom);
    }
                ...
                >
                  {classeEnRenommageId === projet.id ? (
                    <div className="mb-3">
                      ...
                      <input
                        autoFocus
                        value={nomClasseEnEdition}
                        onChange={function (event) {
                          setNomClasseEnEdition(event.target.value);
                        }}
                        onKeyDown={function (event) {
                          if (event.key === "Enter") {
                            enregistrerNouveauNomClasse(projet.id);
                          }

                          if (event.key === "Escape") {
                            annulerRenommageClasse();
                          }
                        }}
                        className="w-full rounded-lg border border-blue-300 px-3 py-2 font-semibold outline-none focus:ring-2 focus:ring-blue-200"
                      />
                      ...
                      <div className="mt-2 flex gap-2">
                        <button
                          ...
                          disabled={!nomClasseEnEdition.trim()}
                          className="flex-1 rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40"
                        >
                          ✓ Enregistrer
                        </button>

                        <button
                          onClick={annulerRenommageClasse}
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <button
                        onClick={function () {
                          commencerRenommageClasse(projet);
                        }}
                        title="Modifier le nom de la classe"
                        aria-label={`Modifier le nom de ${projet.nom}`}
                        className="shrink-0 rounded-lg px-2 py-1 text-sm text-slate-500 hover:bg-slate-100 hover:text-blue-600"
                      >
                        ✏️
                      </button>
                    </div>
